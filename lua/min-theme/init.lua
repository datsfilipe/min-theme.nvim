...
local function set_groups()
	...
	local groups = {
		...
		["@variable.builtin"] = { fg = colors.blue, italic = config.italics.variables or false },
...
function min.colorscheme()
	if vim.version().minor < 8 then
		vim.notify("Neovim 0.8+ is required for min-theme colorscheme", vim.log.levels.ERROR, { title = "Min Theme" })
		return
	end
